...
* `navigation_field.py`: Provides a framework for reactive planning using vector fields without any additional higher-level planning.
...
* `grid_visualization`: Shows an occupancy grid overlay on top of the world. It is useful for showing the effect of different grid resolutions. While not actually a planning example, it provides an example of the world decomposition.
...
* `rrt_planning.py`: Provides a framework for visualizing RRT planning schemes.
